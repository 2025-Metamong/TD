...
public class Tower : MonoBehaviour
{
    ...
    [SerializeField] private Bullet bullet;  // 탄환 컨트롤 및 생성용 스크립트
    ...
    {  
        ...
        this.range = 25f;
        this.upgradeCost = 5;
        this.attackPeriod = 10f; // 초당 10번 공격. 
        this.bullet = gameObject.AddComponent<Bullet>();
    ...
    void Update()
    {
        ...
        */
        ...
        if(this.attack >= 1f){  // 공격 주기에 도달하면
            this.attack = 0f;   // 다음번 공격 주기 계산을 위해 플래그 0으로 초기화.
            this.target = FindTarget(); // 타겟 찾기
            if(this.target != null){
                // Case 1. 타겟을 찾은 경우
                bullet.SetDirections(this); // bullet 이 알아서 발사 될 것.
    ...
    public Transform GetPosition(){     // 타워 오브젝트의 Transform 컴포넌트 리턴.
        return this.position;
    }
    public float GetRange(){    // 타워 사거리 리턴턴
        return this.range;
    }
    ...
    private Transform FindTarget(){
        ...
        */
        float max = 10000000f;
        List<GameObject> monsterList = Monster_Manager.Instance.GetMonsterList();
        float minDistance = max;    
        int minIndex = 0;   

        if(monsterList == null){    // Case 1. 몬스터가 없는 경우
            return null;
        }
        ...
        if(minDistance == max){     // Case 2. 사정거리 내 몬스터 없으면 null 리턴.
            return null;
        }
    ...
    public int GetCost(){   // 타워 설치 비용 리턴.
        return this.cost;
    }

    public int GetUpgradeCost(){    // 타워 업그레이드 비용 리턴.
        return this.upgradeCost;
    }

    public int GetLevel(){      // 타워의 레벨 리턴.
